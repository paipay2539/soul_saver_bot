...
class soulSaverBot:
    ...
    def monitoring(self):
        ...
        global matchTemplate_counter, regex_counter
        '''matchTemplate Vs regex'''
        '''conclusion: regex(1d) faster that matchTemplate(1d)  about 2 time be
        it miss some sample
        ratio about 200 matchTemplate :180 regex(found ratio) '''
        # matchTemplate opencv method
        img_gray = cv2.cvtColor(rescale_img, cv2.COLOR_BGR2GRAY)
        template = cv2.imread('data\pink_neko.png',0)
        template2 = cv2.flip(template, 1)
        w, h = template.shape[::-1]  # 0.028002023696899414
        time_start = time.time()
        ...
        # regex pillow image method
        template = cv2.imread('data\pink_neko.png')
        template2 = cv2.flip(template, 1)
        main = cv2pil(rescale_img)
        sub = cv2pil(template)  # 0.016997098922729492
        sub2 = cv2pil(template2)
        time_start = time.time()
        a = subimg_location(main,sub)
        a2 = subimg_location(main,sub2)
        print((time.time()-time_start))
        print("regex:", subimg_location(main,sub))
        print("regex:", subimg_location(main,sub2))
        if a is not None:
            regex_counter = regex_counter + len(a)
        if a2 is not None:
            regex_counter = regex_counter + len(a2)
        # matchTemplate opencv visualize
        for pt in zip(*loc[::-1]):
            cv2.rectangle(rescale_img, pt, (pt[0] + w, pt[1] + h), (255,0,255), 2)
            print("matchTemplate:",pt)
            matchTemplate_counter = matchTemplate_counter + 1
        ...
        print(matchTemplate_counter, regex_counter)
        # regex pillow image visualize
        try:
            drawRectangle(rescale_img, main, sub)
        except Exception as e:
            pass
        try:
            drawRectangle(rescale_img, main, sub2)
        except Exception as e:
            pass
        ...
        # self.coloring(im)
        cv2.imshow('SoulSaverOnline_cv', rescale_img)
        cv2.waitKey(1)
