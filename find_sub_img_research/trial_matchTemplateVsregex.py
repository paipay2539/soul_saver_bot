...
class soulSaverBot:
    ...
    def monitoring(self):
        ...
        '''conclusion: use matchTemplate because it has more accuracy than regex
        regex must exactly subimg in img, matchTemplate is faster (use 1d color)
        regex maybe faster if use with 1d color algorithm but the disaventage is
        it must use exactly subimg '''
        ...
        sub = cv2pil(template)  # 0.05300593376159668
        time_start = time.time()
